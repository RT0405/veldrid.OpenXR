...
public unsafe class XrVkGraphicsDevice : VkGraphicsDevice
{
    ...
    protected void CreatePhysicalDeviceXR(XrInstance xrInstance, ulong xrSystemID)
    {
        ...
        result = xrGetVulkanGraphicsDeviceKHR(xrInstance, xrSystemID, _instance.Handle, (nint*)&vkPhysicalDevice);
        if (result != XrResult.XR_SUCCESS)
    ...
    {//dont really remember what changed here...
        ...
        List<IntPtr> instanceExtensions = [];//converted these to normal lists since the stacklists seemed to be throwing AccessViolationExceptions
